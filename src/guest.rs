...
use crate::prelude::{Host, HostEditor, PluginFeatures, Project, ThunkContext, State};
...
impl Guest {
    ...
    pub fn guest_context(&mut self) -> ThunkContext {
        let state = self.host.world().system_data::<State>();

        state.initialize_context(self.owner, None)
    }

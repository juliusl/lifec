...
impl Interpreter for Runner {
    ...
    fn interpret(&self, _world: &specs::World, block: &reality::Block) {
        ...
        if block.is_control_block() && block.symbol() == &self.control_symbol {
            // Index all the operations defined in the control block
        ...
        // Index the entity of the receive block
        if self.is_receive_block(block) {

        }

        // Index the entity of the execute block
        if self.is_execute_block(block) {

        }

        // Index the entity of the complete block
        if self.is_complete_block(block) {
...
impl Runner {
    /// Returns true if this is the receive event block,
    ///
    /// Ex. Looks for this block, <``` receive runner> 
    /// 
    fn is_receive_block(&self, block: &reality::Block) -> bool {
        Self::is_event_block(block)
            && block.name() == &self.receive_name
            && block.symbol() == &self.control_symbol
    }

    /// Returns true if this is the execute event block,
    ///
    /// Ex. Looks for this block, <``` execute runner> 
    /// 
    fn is_execute_block(&self, block: &reality::Block) -> bool {
        Self::is_event_block(block)
            && block.name() == &self.execute_name
            && block.symbol() == &self.control_symbol
    }
    ...
    /// Returns true if this is the receive event block,
    ///
    /// Ex. Looks for this block, <``` complete runner> 
    /// 
    fn is_complete_block(&self, block: &reality::Block) -> bool {
        Self::is_event_block(block)
            && block.name() == &self.complete_name
            && block.symbol() == &self.control_symbol
    }

    /// Returns true if this is an event block,
    /// 
    fn is_event_block(block: &reality::Block) -> bool {
        !block.is_control_block() && !block.is_root_block()
    }
}
